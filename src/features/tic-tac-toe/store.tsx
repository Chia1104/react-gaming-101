...
import { createDevtools } from '@/stores/middleware/create-devtools';
...
export type TicTacToeStoreApi = StateCreator<TicTacToeStore, [['zustand/devtools', never]], [], TicTacToeStore>;
...
export const TicTacToeStoreContext = createContext<StoreApi<TicTacToeStore> | null>(null);

export const useTicTacToeStore = <T,>(selector: (store: TicTacToeStore) => T, name = 'useTicTacToeStore'): T => {
...
export const TicTacToeStoreProvider = ({ children, initialState }: TicTacToeStoreProviderProps) => {
	...
	return <TicTacToeStoreContext value={store.current}>{children}</TicTacToeStoreContext>;
};
