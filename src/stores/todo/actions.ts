...
import { nanoid } from '@/libs/utils/uuid';
...
export const createTodoActions: StateCreator<InitialState, [], [], TodoActions> = set => ({
	addTodo: (text: string) => {
		set(state => ({ items: [...state.items, { id: nanoid(), text, completed: false }] }));
	},
